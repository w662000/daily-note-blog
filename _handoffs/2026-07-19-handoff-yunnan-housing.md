---
layout: default
title: 交接文档 · 云南旅居房分析系统
date: 2026-07-19 23:30:00 +0800
---

# 云南旅居房分析系统（yunnan-housing）— 交接文档（人读）

> 更新于 2026-07-19（含 7.24–25 无锡版复用同一套架构的延伸）。项目根目录：`D:\AI work\workbuddy\2026-07-19-09-49-20\yunnan-housing\`
> 给接手的同学看。AI/agent 接手请直接读同目录的 `HANDOFF.md` / `HANDOFF_AGENT.md`（无锡交付版，架构同源）。

---

## 0. 一句话成果

基于 Cloudflare 全栈（Workers + D1 + KV + Pages）搭了房价采集看板：**买房系统**爬 58 同城（云南 16 城 + 无锡 7 区）+ 贝壳 ke.com，**租房系统**爬 58 同城租房（云南 + 无锡 7 区），数据落 D1、看板可看全量，沉淀 11 条反爬铁律。这是后续「无锡 58 爬虫」(handoff #5) 的**前身与架构母体**。
...
- 数据源：原计划贝壳 km.ke.com（昆明 5 片区），实测贝壳/链家强制登录、优居/Q房连不上、房天下空壳 → **58 同城可用**，主力切 58；另建贝壳 `beike_*` 三件套（ke.com 二手房）。
- 目标城市：云南 16 个地级行政区（昆明/曲靖/玉溪/…/迪庆州），无锡 7 区（梁溪/惠山/新吴/江阴/宜兴/滨湖/锡山）。
...
- **2026-07-24~25**（来源 `2026-07-25.md`）：无锡买房 `wx-houseing` 7 区全落 D1（3685 条）；租房 `wxzf` 7 区（7466 条），复用本架构，详见 handoff #5。
